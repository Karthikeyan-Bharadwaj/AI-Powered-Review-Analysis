...
const platformConfig = {
  ...
  bestbuy: {
    ...
    titleClass: "gradient-text-orange",
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        <h1 className={cn("text-4xl md:text-5xl font-bold", config.titleClass)}>
          {config.title}
        </h1>
      ...
      <Card className="p-8 bg-card/50 backdrop-blur-sm border-border shadow-[var(--shadow-card)]">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {isLoading && (
            <p className={cn("text-center text-xs animate-pulse", config.accent)}>
